...
class TikTikenizer:
    ...
    @staticmethod
    def encode(small_llm:"Small_LLM_Model",path_to_merges_file:Path,text:str):
        ...
        try:
            bpe_ranks = {}
            with open(path_to_merges_file, "r", encoding="utf-8") as file:
        ...
        def separate_to_pairs(unicode_chars:list) -> list: #sepatere to pairs for BPE
            pairs_arr = []
            i = 0
        ...
        def get_higher_priority(pairs_arr) -> list | None:
            best_pair = None
            best_rank = float('inf')
        ...
        while True:
            ...
            for i in range(len(new_word_arr)):
                if new_word_arr[i] == higher[0] and new_word_arr[i+1 ] == higher[1]:
        ...
        try:
            ...
            for token in new_word_arr:
                tokinized.append(vocab.get(token))
    ...
    def decode(small_llm:"Small_LLM_Model",tokenids_list:list[int]) -> str:
        ...
        for token in tokenids_list:
            chars.append(inverted_vocab.get(token))
        text = []
        for c in "".join(chars):
            if c == "Ġ":
                text.append(" ")  # Space
...
if __name__ == "__main__":
    ...
    tiktik = TikTikenizer()
    text = " hi hi"
    res_enc = tiktik.encode(small_llm,small_llm.get_path_to_merges_file(),text)
